...
def pizza(request):
    if request.user.is_authenticated():
        ...
        admin_list = list(Pizza.objects.filter(pizza_id=order_list).select_related().values())
        uorder_list = Orders.objects.filter(user_name=str(request.user))
        user_list = list(Pizza.objects.filter(pizza_id=uorder_list).select_related().values())
